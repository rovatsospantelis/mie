...
const phone = site.social.whatsapp
const text = encodeURIComponent(`Γεια σας! Θα ήθελα πληροφορίες.`)
...
<template>
  <a
    ...
    rel="noopener"
    aria-label="WhatsApp"
    @click="trackContact('whatsapp', 'float')"
    class="fixed right-4 z-40 flex h-14 w-14 items-center justify-center rounded-full text-white shadow-lg transition-transform hover:scale-105"
    style="bottom: calc(6.5rem + env(safe-area-inset-bottom)); background: #25D366"
  >
